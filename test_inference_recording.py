...
class TestRecordInference:
    ...
    async def test_record_inference_creates_log_and_upserts_subscription(self):
        ...
        with patch("main.get_model_meta_by_mcp_tool", new_callable=AsyncMock, return_value=model):
            with patch("main.upsert_subscription_stats", new_callable=AsyncMock) as mock_upsert:
                ...
                await _record_inference(session, "yolo2026_detect", response, mode="HTTP", payload_size_bytes=1024)

        session.add.assert_called_once()
        added_log = session.add.call_args[0][0]
        assert isinstance(added_log, CommunicationLog)
        assert added_log.model_id == "yolo2026"
        ...
        mock_upsert.assert_awaited_once_with(session, model_id="yolo2026", model_name="YOLO2026", latency_ms=12.0, success=True)
    ...
    async def test_record_inference_survives_db_exception(self):
        ...
        with patch("main.get_model_meta_by_mcp_tool", new_callable=AsyncMock, return_value=model):
            ...
            # Should NOT raise
            await _record_inference(session, "yolo2026_detect", response, mode="HTTP")
...
class TestUpsertSubscriptionStats:
    ...
    async def test_creates_subscription_when_none_exists(self):
        ...
        sub = await upsert_subscription_stats(session, "yolo2026", "YOLO2026", latency_ms=12.0, success=True)

        assert isinstance(sub, Subscription)
        assert sub.model_id == "yolo2026"
    ...
    async def test_updates_existing_subscription(self):
        ...
        sub = await upsert_subscription_stats(session, "yolo2026", "YOLO2026", latency_ms=25.0, success=True)
    ...
    async def test_failure_lowers_success_rate(self):
        ...
        sub = await upsert_subscription_stats(session, "yolo2026", "YOLO2026", latency_ms=25.0, success=False)
            ...
            with patch.object(main, "_record_inference", new_callable=AsyncMock) as mock_record:
                with patch.object(main, "get_db") as mock_get_db:
                    ...
                    with TestClient(main.app) as client:
                        res_call = client.post("/call", json={"tool": "yolo2026_detect", "arguments": {"image": "test"}})
                        ...
                        res_msg = client.post(
                            ...
                            json={
                                ...
                                "id": 1,
                                "method": "tools/call",
                                "params": {"name": "yolo2026_detect", "arguments": {"image": "test"}},
                            },
                        )
                    ...
                    assert call_args[0][0][1] == "yolo2026_detect"
                    assert call_args[0][1]["mode"] == "HTTP"
                    assert call_args[1][0][1] == "yolo2026_detect"
