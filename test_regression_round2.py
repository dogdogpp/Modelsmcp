"""Round-2 regression test suite for develop branch (HEAD: d3e3b0f7)."""
import os
import sys
import base64
import tempfile
...
class TestMcpServer:
    def test_whisper_real_handler_in_non_mock(self):
        """Verify that real whisper handler is bridged in non-mock mode."""
        os.environ["DEEPMCP_API_KEY"] = ""
        ...
        # The bridged handler should be the adapter, not the raw mock
        res = handler({"audio": "test.wav", "language": "en"})
        # Since whisper is actually installed in this env, the real handler will try to load model.
        # It should fail on the fake path, but the important thing is it's NOT the mock result.
    ...
    def test_whisper_mock_fallback_when_not_installed(self):
        """Verify mock fallback when real handler is unavailable."""
        os.environ["DEEPMCP_API_KEY"] = ""
        os.environ["DEEPMCP_MOCK_MODE"] = "false"
        os.environ["DEEPMCP_CAMERA_ENABLED"] = "false"
        import config
        ...
        import server.mcp.server as mcp_server
        importlib.reload(mcp_server)
        # Simulate missing handler by making get_handler return None
        with patch("server.models.get_handler", return_value=None):
            mcp_server.init_tools(mock_mode=False, camera_manager=None)
            handler = mcp_server._TOOL_HANDLERS.get("whisper_transcribe")
            res = handler({"audio": "test.wav", "language": "en"})
            assert res.result and "欢迎使用 DeepMCP" in str(res.result)
...
class TestFrontendBuild:
    ...
    def test_camera_panel_no_hardcoded_dev_key(self):
        ...
        # Also verify authHeaders only sends key when present
        assert "if (API_KEY) headers" in source or "if(API_KEY)headers" in source.replace(" ", "")
